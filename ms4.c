...
void initPlayer(struct Player *player)
{
    ...
    scanf("%35s", (*player).playerName1);;
	while('\n'!=getchar());
}

void playGame(unsigned int boardSize, struct Player players[], unsigned int playerCount)
{
	unsigned int roll,rollval[3] = {0,0,0},i,j,z,stop,x,score,game,lost;
    int won = 2, over = 0;
    int out[playerCount];
    int outcount = 0;
    do
    {
        ...
        for(i=0;i<playerCount;i++)
        {
            ...
            if(stop == 0)
            {
                ...
                printf("\n");
                printf("%s\'s (%c) turn, how many dice will you roll? ",players[i].playerName1,players[i].playerName);
                roll = playerRoll();
                for(j=0;j<roll;j++)
                    rollval[j] = 1 + rand() % (6);
                if(roll == 1)
            ...
            {
                game++;
                addToLeaderboard(game,&players[i]);
                break;
            }
        }
    }while(won!=1);
...
int checkout(struct Player* player)
{
    ...
    {
		printf("__\n  \\_______\n   \\++++++|\n    \\=====|\n    0---  0\n\n");
        printf("%s (%c) checkedout for $%d\n\n",(*player).playerName1,(*player).playerName,value);
...
//  High scores winner is stored here.....
void addToLeaderboard(int game, struct Player* winner)
{
	FILE * fp;
	fp = fopen("leaderboard.dat" , "a");
	fprintf(fp, "GAME:%d %s ( %c ) - %d\n", game, winner->playerName1, winner->playerName, winner->playerScore);
	fclose(fp);
}
